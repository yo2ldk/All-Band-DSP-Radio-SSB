...
#define MIN_ELAPSED_VOLbut_TIME     1500
...
Button bt[] = {      
  ...
  { "BROAD" ,  1 , "SEEK +" , 1 , Xbut1 , Ybut1  },    
  { "MODE"  ,  2 , ""       , 2 , Xbut2 , Ybut2  },
  { "AGC"   ,  3 , "RDS"    , 3 , Xbut3 , Ybut3  }, 
  { "VOL"   ,  4 , "AGC"    , 4 , Xbut4 , Ybut4  }, 
  { "STEP"  ,  5 , "INFO"   , 5 , Xbut5 , Ybut5  },    
  { "FILTER",  6 , ""       , 6 , Xbut6 , Ybut6  },    
  { "NEXT"  ,  7 , "BACK"   , 7 , Xbut7 , Ybut7  },     
...
Band band[] = {
  ...
  {"SW ALL", SW_BAND_TYPE,  AM, 1730, 30000, 15500, 5}  // Whole SW    29
};
//======================================================= End THE Band Definitions     ========================

//======================================================= FM Presets     ======================================

// Here you can add your favorite FM station in Memory
...
//=======================================================================================  
  ...
  for (int i = 0; i < 10; i++) {
    ...
    uint16_t calData[5] = {250, 3550, 400, 3550,1 };   //{ 387, 3530, 246, 3555, 7 };
  ...
  delay(500);
  tft.setCursor(90, 30);
  tft.setFreeFont(&URW_Palladio_L_Bold_20);
  tft.setTextSize(1);
  ...
  tft.println(" v1.1 by YO2LDK - Alex");
...
//=======================================================================================
  ...
  tft.fillRect(Xsmtr + 50, Ysmtr + 38 , (2 + spoint), 6, TFT_GREEN);
  tft.fillRect(Xsmtr + 52 + spoint, Ysmtr + 38 , 212 - (2 + spoint), 6, TFT_DARKGREEN);
}

//=======================================================================================
void VolumeIndicator(int vol) {
//=======================================================================================
  vol = map(vol, 20, 100, 50, 212);
  tft.fillRect(XVolInd + 55, YVolInd + 16 ,(2 + vol), 5, TFT_RED);
  tft.fillRect(XVolInd + 55 + vol, YVolInd + 16 , 212 - (2 + vol), 5, TFT_GREEN);
}
//=======================================================================================
void loop() {
//=======================================================================================
      ...
      if (FREQbut == false) {
        for (int n = 0 ; n < 12; n++) { // which keys are pressed?
          if ((x > ((kp[n].Xkeypos) + (kp[n].Xkeypnr))) and (x < ((kp[n].Xkeypos) + (kp[n].Xkeypsr) + (kp[n].Xkeypnr))) and (y > ((kp[n].Ykeypos) + (kp[n].Ykeypnr))) and (y < ((kp[n].Ykeypos) + (kp[n].Ykeypsr) + (kp[n].Ykeypnr)))) {
            tft.fillRect((kp[n].Xkeypos + kp[n].Xkeypnr + 3) , (kp[n].Ykeypos + kp[n].Ykeypnr + 3), (kp[n].Xkeypsr - 3) , (kp[n].Ykeypsr - 3), TFT_WHITE);
            tft.setTextColor(TFT_BLUE, TFT_WHITE ); // Temporary white button when touched
            tft.setFreeFont(&Serif_bold_20);
            tft.setTextSize(1);
            ...
            Beep(1, 0);
            delay(100);
            tft.fillRect((kp[n].Xkeypos + kp[n].Xkeypnr + 3) , (kp[n].Ykeypos + kp[n].Ykeypnr + 3), (kp[n].Xkeypsr - 3) , (kp[n].Ykeypsr - 3), TFT_BLUE);
...
//=======================================================================================
  if (( currentMode == FM) and((FirstLayer) or (ThirdLayer))){
      if ( currentFrequency != previousFrequency ) {
        ...
        //stationName = '\0';
        tft.fillRect(XFreqDispl + 110, YFreqDispl + 14, 10,10, TFT_BLACK);  // clear RDS text
      }
      if ((RDS) and  (NewSNR >= 12)) checkRDS(); 
      else  tft.fillRect(XFreqDispl + 100, YFreqDispl + 14, 10, 10, TFT_BLACK); // clear RDS text
  }
}
...
//=======================================================================================
  if ((  currentMode == FM ) and ((FirstLayer) or (ThirdLayer))) {
    ...
    tft.setTextDatum(BC_DATUM);
    tft.setTextPadding(0);
    tft.fillRect(XFreqDispl + 205, YFreqDispl + 10 , 10, 12, TFT_BLACK); // STEREO MONO
  ...
  if ((FirstLayer) or (ThirdLayer)) {  // dBuV and volume at freq. display
   // tft.fillRect(XFreqDispl + 7, YFreqDispl + 75 , 173, 10, TFT_BLACK);
    ...
    tft.drawString("RSSI= "+ String(NewRSSI) + "dB" , XFreqDispl + 5, YFreqDispl + 70);
    tft.setFreeFont(NULL);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.setFreeFont(&Serif_plain_15);
    tft.setTextSize(1);
    tft.setTextDatum(TR_DATUM);
    tft.drawString("SNR= " + String(NewSNR) + "dB", XFreqDispl + 315, YFreqDispl + 70);
    tft.setFreeFont(NULL);
  }
  VOLbutoff();
}
...
//=======================================================================================
  for (int n = 0 ; n <= lastButton; n++) {
    tft.fillRect(bt[bt[n].ButtonNum].XButos + Xbutst, bt[bt[n].ButtonNum].YButos + Ybutst , Xbutsiz , Ybutsiz, TFT_VIOLET);
    tft.fillRect((bt[bt[n].ButtonNum].XButos + Xbutst + 3) , (bt[bt[n].ButtonNum].YButos + Ybutst + 3), (Xbutsiz - 3) , (Ybutsiz - 3), TFT_BLUE);
    tft.setTextColor(TFT_CYAN, TFT_BLUE);
...
//=======================================================================================
  ...
  for (int n = 0 ; n <= lastButton; n++) {
    tft.fillRect(bt[bt[n].ButtonNum1].XButos + Xbutst, bt[bt[n].ButtonNum1].YButos + Ybutst , Xbutsiz , Ybutsiz, TFT_VIOLET);
    tft.fillRect((bt[bt[n].ButtonNum1].XButos + Xbutst + 3) , (bt[bt[n].ButtonNum1].YButos + Ybutst + 3), (Xbutsiz - 3) , (Ybutsiz - 3), TFT_BLUE);
    tft.setTextColor(TFT_CYAN, TFT_BLUE);
    tft.setFreeFont(&Serif_bold_15);
    tft.setTextSize(1);
    tft.setTextDatum(BC_DATUM);
...
//=======================================================================================
  tft.setFreeFont(&Serif_bold_10);
  tft.setTextSize(1);
  tft.fillRect(XVolInd, YVolInd, 320, 30, TFT_VIOLET);
  tft.fillRect(XVolInd + 3, YVolInd + 3, 315, 25, TFT_BLACK);
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  tft.setCursor(XVolInd +  55, YVolInd + 14);
  tft.print("0");
  tft.setCursor(XVolInd +  100, YVolInd + 14);
  tft.print("25");
  tft.setCursor(XVolInd + 150, YVolInd + 14);
  tft.print("50");
  tft.setCursor(XVolInd + 195, YVolInd + 14);
  tft.print("75");
  tft.setCursor(XVolInd + 240, YVolInd + 14);
  tft.print("100");
  tft.setFreeFont(NULL);
}
...
//=======================================================================================
  ...
  for (int i = 0; i < 10; i++) {
    ...
    IStr = String(i);
    tft.setCursor((Xsmtr + 50 + (i * 12)), Ysmtr + 17);
    tft.print(i);
  }
  for (int i = 1; i < 7; i++) {
    tft.fillRect((Xsmtr + 162 + (i * 16)), Ysmtr + 24, 4, 8, TFT_RED);
    IStr = String(i * 10);
    tft.setCursor((Xsmtr + 160 + (i * 16)), Ysmtr + 17);
...
//=======================================================================================
  ...
  tft.fillRect(bt[bt[5].ButtonNum].XButos + Xbutst, bt[bt[5].ButtonNum].YButos + Ybutst , Xbutsiz , Ybutsiz, TFT_VIOLET);
  tft.fillRect((bt[bt[5].ButtonNum].XButos + Xbutst + 3) , (bt[bt[5].ButtonNum].YButos + Ybutst + 3), (Xbutsiz - 3) , Ybutsiz - 3, VOLbutcol);
...
if (ThirdLayer)  {  
  ...
  tft.fillRect(bt[bt[4].ButtonNum1].XButos + Xbutst, bt[bt[4].ButtonNum1].YButos + Ybutst , Xbutsiz , Ybutsiz, TFT_VIOLET);
  tft.fillRect((bt[bt[4].ButtonNum1].XButos + Xbutst + 3) , (bt[bt[4].ButtonNum1].YButos + Ybutst + 3), (Xbutsiz - 3) , Ybutsiz - 3, AGCgainbutcol);
...
//=======================================================================================
  ...
  tft.fillRect(bt[bt[3].ButtonNum1].XButos + Xbutst, bt[bt[3].ButtonNum1].YButos + Ybutst , Xbutsiz , Ybutsiz, TFT_VIOLET);
  tft.fillRect((bt[bt[3].ButtonNum1].XButos + Xbutst + 3) , (bt[bt[3].ButtonNum1].YButos + Ybutst + 3), (Xbutsiz - 3) , Ybutsiz - 3, RDSbutcol);
...
//=======================================================================================
  ...
  tft.fillRect(bt[bt[4].ButtonNum].XButos + Xbutst, bt[bt[4].ButtonNum].YButos + Ybutst , Xbutsiz , Ybutsiz, TFT_VIOLET);
  tft.fillRect((bt[bt[4].ButtonNum].XButos + Xbutst + 3) , (bt[bt[4].ButtonNum].YButos + Ybutst + 3), (Xbutsiz - 3) , Ybutsiz - 3, MUTEbutcol);
...
//=======================================================================================
  ...
  tft.fillRect(bt[bt[3].ButtonNum].XButos + Xbutst, bt[bt[3].ButtonNum].YButos + Ybutst , Xbutsiz , Ybutsiz, TFT_VIOLET);
  tft.fillRect((bt[bt[3].ButtonNum].XButos + Xbutst + 3) , (bt[bt[3].ButtonNum].YButos + Ybutst + 3), (Xbutsiz - 3) , Ybutsiz - 3, AGCbutcol);
...
//=======================================================================================
  int BFObutcol;
  if (bfoOn) BFObutcol = TFT_BLUE;
  else BFObutcol = TFT_BLUE;
  tft.fillRect(bt[bt[1].ButtonNum].XButos + Xbutst, bt[bt[1].ButtonNum].YButos + Ybutst , Xbutsiz , Ybutsiz, TFT_VIOLET);
  tft.fillRect((bt[bt[1].ButtonNum].XButos + Xbutst + 3) , (bt[bt[1].ButtonNum].YButos + Ybutst + 3), Xbutsiz - 3 , (Ybutsiz - 3), BFObutcol);
...
//=======================================================================================
  ...
  for (int n = 0 ; n <= lastHam; n++) {
    ...
    tft.fillRect((bn[n].Xbandos + bn[n].Xbandnr + 3) , (bn[n].Ybandos) + (bn[n].Ybandnr + 3), (bn[n].Xbandsr - 3) , (bn[n].Ybandsr - 3), TFT_BLUE);
    tft.setTextColor(TFT_CYAN, TFT_BLUE);
    tft.setFreeFont(&Serif_bold_20);
    tft.setTextSize(1);
...
//=======================================================================================  
  ...
  for (int n = 0 ; n <= lastBroad; n++) {
    ...
    tft.fillRect((bb[n].Xbbandos + bb[n].Xbbandnr + 3) , (bb[n].Ybbandos) + (bb[n].Ybbandnr + 3), (bb[n].Xbbandsr - 3) , (bb[n].Ybbandsr - 3), TFT_BLUE);
    tft.setFreeFont(&Serif_bold_20);
    tft.setTextSize(1);
...
//=======================================================================================
  ...
  for (int n = 0 ; n <= lastStep; n++) {
    ...
    tft.fillRect( (sp[n].Xstepos + 3), sp[n].Ystepos + 3 + (sp[n].Ystepnr) , sp[n].Xstepsr - 3 , sp[n].Ystepsr - 3, TFT_BLUE);
    tft.setTextColor(TFT_CYAN, TFT_BLUE);
    tft.setFreeFont(&Serif_bold_20);
    tft.setTextSize(1);
...
//=======================================================================================
  ...
  tft.setTextPadding(0);  
  for (int n = 1 ; n <= lastMod; n++) {
    tft.fillRect( (md[n].Xmodos), (md[n].Ymodos) + (md[n].Ymodnr), (md[n].Xmodsr), (md[n].Ymodsr), TFT_VIOLET);
    tft.fillRect( (md[n].Xmodos + 3), md[n].Ymodos + 3 + (md[n].Ymodnr) , md[n].Xmodsr - 3 , md[n].Ymodsr - 3, TFT_BLUE);
    tft.setTextColor(TFT_CYAN, TFT_BLUE);
    tft.setFreeFont(&Serif_bold_20);  
    tft.setTextSize(1);
    tft.drawString(bandModeDesc[md[n].Modenum], (md[n].Xmodos) + 100, (md[n].Ymodos) + (md[n].Ymodnr) + 40);
    tft.setFreeFont(NULL);
  }
}
...
//=======================================================================================
  ...
  for (int n = 0 ; n < nrbox; n++) {
    ...
    tft.fillRect( (bw[n].Xos + 3), bw[n].Yos + 3 + (bw[n].Ynr) , bw[n].Xsr - 3 , bw[n].Ysr - 3, TFT_BLUE);
    tft.setTextColor(TFT_CYAN, TFT_BLUE);
    tft.setFreeFont(&Serif_bold_15);  
    tft.setTextSize(1);
  ...
  tft.setFreeFont(&Serif_bold_15);  
  tft.setTextSize(1);
  if ( currentMode == AM)  tft.drawString("AM Bandw. in kHz"  , XfBW + 60, YfBW - 22);
  if ( currentMode == USB) tft.drawString("USB Bandw. in kHz" , XfBW + 60, YfBW - 22);
  if ( currentMode == LSB) tft.drawString("LSB Bandw. in kHz" , XfBW + 60, YfBW - 22);
  if ( currentMode == SYNC)  tft.drawString("SYNC Bandw. in kHz", XfBW + 50, YfBW - 22);
  tft.setFreeFont(NULL);
}
...
//=======================================================================================
  ...
  while (x == 0) {
    ...
    tft.drawString("FILTRU SSB : " + String(bandwidthSSB[bwIdxSSB]) + " kHz", 150, 150);
    tft.drawString("FILTRU AM : " + String(bandwidthAM[bwIdxAM]) + " kHz", 150, 160);
    ...
    tft.setFreeFont(&Serif_bold_15);
    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.drawString("SOFTWARE V2.01", 160, 205);
    tft.drawString("BY RICARDO - GERT - THIAGO - Alex", 160, 225);
    tft.setFreeFont(NULL);
...
//=======================================================================================
  if ((FirstLayer) or (ThirdLayer)) {
    // tft.drawString(stationName, XFreqDispl + 126, YFreqDispl + 62);
     tft.setCursor(XFreqDispl + 126, YFreqDispl + 72);
...
void showFrequency() {
  ...
  if (bfoOn) {  
     ...
     showContent(XFreqDispl + 175, YFreqDispl + 55, bufferVFO, tmpVFO, &DSEG7_Classic_Mini_Bold_20, TFT_ORANGE, 18);
     showContent(XFreqDispl + 270, YFreqDispl + 40, bufferAux, "Hz", &Serif_bold_10, TFT_GREEN, 10);
     showContent(XFreqDispl + 270, YFreqDispl + 55, bufferAux, "BFO", &Serif_bold_15, TFT_RED, 12);
     untFreq = formatFrequency(tmpFrequency);
     showContent(XFreqDispl +80, YFreqDispl +55, bufferFrequency, tmpFrequency, &URW_Palladio_L_Bold_40, TFT_CYAN, 18);
  } else {
     untFreq = formatFrequency(tmpFrequency);
     showContent(XFreqDispl + 105, YFreqDispl + 62, bufferFrequency, tmpFrequency, &URW_Palladio_L_Bold_55, TFT_CYAN, 26);
     if (band[bandIdx].bandType == FM_BAND_TYPE) 
     tft.setFreeFont(&URW_Palladio_L_Bold_20);
     tft.drawChar(XFreqDispl + 88 + 94, YFreqDispl + 62, (si4735.isCurrentTuneFM())?',' :' ', TFT_CYAN, TFT_BLACK, 1);
     //showContent(XFreqDispl + 155, YFreqDispl + 62, bufferAux, ",", &Serif_bold_20, TFT_RED, 12);
     tft.setFreeFont(NULL);
     showContent(XFreqDispl + 250, YFreqDispl + 50, bufferUnit, untFreq, &Serif_bold_15, TFT_GREEN, 14);
     showContent(XFreqDispl + 45, YFreqDispl + 50, bufferBandName, (char *)band[bandIdx].bandName, &Serif_bold_20, TFT_RED, 12);
  }
...
void FreqDispl()
{
  ...
  if (VOLbut) {
      ...
      showContent(XFreqDispl + 20, YFreqDispl + 60, bufferVolume, tmpAux, &DSEG7_Classic_Mini_Bold_30, TFT_CYAN, 20);
      showContent(XFreqDispl + 80, YFreqDispl + 50, bufferAux, " VOLUME", &Serif_plain_15, TFT_GREEN, 20);
  } else if (AGCgainbut) {
      sprintf(tmpAux,"%2.2d",currentAGCgain);
      showContent(XFreqDispl + 20, YFreqDispl + 50, bufferAux, tmpAux, &DSEG7_Classic_Mini_Bold_30, TFT_CYAN, 20);
      showContent(XFreqDispl + 80, YFreqDispl + 50, bufferAux, "ATT SET", &Serif_plain_15, TFT_GREEN, 20);
  } else {
    showFrequency();
  }
...
//=======================================================================================
  ...
  tft.fillRect(XFreqDispl, YFreqDispl, 320, 90, TFT_VIOLET);
  tft.fillRect(XFreqDispl + 3, YFreqDispl + 3, 315, 82, TFT_BLACK);  
  FreqDispl();
  ...
  if (band[bandIdx].bandType != FM_BAND_TYPE) {
    ...
    tft.setTextSize(1);
    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.drawString(bandModeDesc[currentMode], XFreqDispl + 65, YFreqDispl + 22);
    tft.setTextPadding(tft.textWidth("2.2kHz"));
    if (currentMode == AM) BWtext = bandwidthAM[bwIdxAM];
    else BWtext = bandwidthSSB[bwIdxSSB];
    tft.setTextSize(1);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("FT", XFreqDispl + 120, YFreqDispl + 22);
    tft.drawString(BWtext + "kHz", XFreqDispl + 170, YFreqDispl + 22);
    tft.setTextSize(1);
    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.drawString("ST", XFreqDispl + 248, YFreqDispl + 22);    
    tft.drawString(String(band[bandIdx].currentStep) + "kHz", XFreqDispl + 286, YFreqDispl + 22);
    tft.setFreeFont(NULL);
  }
}
...
//=======================================================================================
  ...
  tft.setTextSize(1);
  tft.setTextDatum(BC_DATUM);
  tft.drawString("AGC", XFreqDispl + 20, YFreqDispl + 15);
  si4735.getAutomaticGainControl();
  if (si4735.isAgcEnabled()) {
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("ON", XFreqDispl + 19, YFreqDispl + 30);    
  } else {
    if (AGCgain == 0)   {
      tft.setTextColor(TFT_RED, TFT_BLACK);
      tft.drawString("OFF", XFreqDispl + 20, YFreqDispl + 55);
    } else {
      tft.drawString(String(currentAGCgain), XFreqDispl + 15, YFreqDispl + 75); 
      tft.setFreeFont(NULL); 
    }
  }
} 
...
if (band[bandIdx].bandType != FM_BAND_TYPE) {
    ...
    tft.drawString("BFO", XFreqDispl + 20, YFreqDispl + 105);
    tft.setTextPadding(tft.textWidth("88"));  
    if (bfoOn) {
      tft.setTextColor(TFT_RED, TFT_BLACK);
      tft.drawString(String(currentBFOStep), XFreqDispl + 18, YFreqDispl + 115);
      tft.setTextColor(TFT_BLUE, TFT_BLACK);
    } else {
      tft.setTextColor(TFT_BLUE, TFT_BLACK);
      tft.drawString("  ", XFreqDispl + 20, YFreqDispl + 125);
      tft.setFreeFont(NULL);
    }
  }
}   
